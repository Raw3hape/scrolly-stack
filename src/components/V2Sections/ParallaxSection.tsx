...
export default function ParallaxSection({ section, children }: ParallaxSectionProps) {
  ...
  const isCinematicMission =
    section.type === 'mission' && 'backgroundUrl' in section && !!section.backgroundUrl;
  const isExpand = meta.expand || isCinematicMission;
  const ref = useParallax<HTMLElement>(isExpand ? { completionFactor: 0.85 } : undefined);
